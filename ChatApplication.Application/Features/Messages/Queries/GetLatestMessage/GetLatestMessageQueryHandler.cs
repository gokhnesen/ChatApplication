...
namespace ChatApplication.Application.Features.Messages.Queries.GetLatestMessage
{
    public class GetLatestMessageQueryHandler : IRequestHandler<GetLatestMessageQuery, GetLatestMessageQueryResponse>
    {
        ...
        {
            _logger.LogInformation("En son mesaj getiriliyor: {UserId1} <-> {UserId2}", request.UserId1, request.UserId2);

            var latestMessage = await _messageReadRepository.GetLatestMessageAsync(request.UserId1, request.UserId2, cancellationToken);

            if (latestMessage == null)
            {
                _logger.LogInformation("En son mesaj bulunamadı: {UserId1} <-> {UserId2}", request.UserId1, request.UserId2);
